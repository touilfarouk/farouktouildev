...
function initcodacom() {
  ...
  function hoverdirInit() {
    $(
      ".horizontal-grid-wrap .portfolio_item  , .gallery-items .gallery-item"
    ).each(function () {
      $(this).hoverdir();
    });
  }
  ...
  $(
    "a , .btn ,   textarea,   input  , .leaflet-control-zoom , .aside-show_cf , .close-contact_form , .closedet_style "
  ).on({
    mouseenter: function () {
  ...
  $(".page-scroll-nav_wrap ul li a  , .gallery-filters a").on(
    "click",
    function () {
      if ($(window).width() < 565) {
